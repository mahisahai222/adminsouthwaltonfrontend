...
const AuthList = () => {
  ...
  const fetchAuths = async () => {
    try {
      const response = await axios.get('http://44.196.64.110:8132/api/auths/');
  ...
  const handleAddAuth = async () => {
    try {
      const response = await axios.post('http://44.196.64.110:8132/api/auths/website', { username, email, password });
  ...
  const handleDeleteAuth = async (id) => {
    try {
      await axios.delete(`http://44.196.64.110:8132/api/auths/${id}`);
