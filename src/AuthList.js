...
const AuthList = () => {
  ...
  const fetchAuths = async () => {
    try {
      const response = await axios.get('http://18.209.197.35:8132/api/auths/');
  ...
  const handleAddAuth = async () => {
    try {
      const response = await axios.post('http://18.209.197.35:8132/api/auths/website', { username, email, password });
  ...
  const handleDeleteAuth = async (id) => {
    try {
      await axios.delete(`http://18.209.197.35:8132/api/auths/${id}`);
