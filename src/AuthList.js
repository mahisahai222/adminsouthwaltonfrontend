...
const AuthList = () => {
  ...
  const fetchAuths = async () => {
    try {
      const response = await axios.get('http://3.111.163.2:8132/api/auths/');
  ...
  const handleAddAuth = async () => {
    try {
      const response = await axios.post('http://3.111.163.2:8132/api/auths/website', { username, email, password });
  ...
  const handleDeleteAuth = async (id) => {
    try {
      await axios.delete(`http://3.111.163.2:8132/api/auths/${id}`);
