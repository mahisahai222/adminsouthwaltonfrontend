...
const BookManageList = () => {
  ...
  const fetchBookings = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://54.236.98.193:8132/api/book', {
        params: {
          search: searchTerm,
          page,
          limit,
  ...
  const fetchAvailableDrivers = async () => {
    try {
      const response = await axios.get('http://54.236.98.193:8132/api/driver/'); // Update API endpoint if necessary
  ...
  const deleteBooking = async (id) => {
    try {
      await axios.delete(`http://54.236.98.193:8132/api/book/${id}`);
  ...
  const assignDriver = async () => {
    ...
    try {
      const response = await axios.post('http://54.236.98.193:8132/api/driver/assignDriver', requestData);
  ...
  const handleAddBooking = async () => {
    ...
    try {
      const response = await axios.post('http://54.236.98.193:8132/api/book/create', formData);
      console.log('Booking added:', response.data);

      const updateResponse = await axios.put(`http://54.236.98.193:8132/api/reserve/reservation/${reservationId}`, {
        booking: true
      });
  ...
  const fetchPanelBookings = async () => {
    ...
    try {
      const response = await fetch(`http://54.236.98.193:8132/api/book/bookfromPanel?page=${panelPage}&limit=${panelLimit}&search=${searchQuery}`);
  ...
  const fetchReservations = async () => {
    try {
      const response = await fetch('http://54.236.98.193:8132/api/reserve/reservations/fromPanel'); // Replace with your API URL
      const data = await response.json();
  ...
  const sendPaymentMail = async (booking) => {
    ...
    try {
      const response = await fetch('http://54.236.98.193:8132/api/send-rental-agreement', {
        method: 'POST',
        headers: {
