...
const BookManageList = () => {
  ...
  const addBooking = async () => {
    try {
      ...
      await axios.post('http://44.196.192.232:8132/api/book/create', newBooking);
      fetchBookings();
      setVisible(false);
    } catch (error) {
  ...
  const updateBooking = async () => {
    try {
      ...
      await axios.put(`http://44.196.192.232:8132/api/book/${currentBooking._id}`, updatedBooking);
      fetchBookings();
      setVisible(false);
    } catch (error) {
  ...
  //   try {
  //     console.log("running")
  //     const response = await axios.post('http://44.196.192.232:8132/api/book/available-by-drop-date', { bdropDate: bdropDate });
  ...
  const assignDriver = async () => {
    try {
      ...
      const response = await axios.put(`http://44.196.192.232:8132/api/driver/assignDriver/${currentDriver}`, requestData)
