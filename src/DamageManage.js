...
const DamageManage = () => {
  ...
  const fetchDamageManageData = async (page = 1, limit = 5, search = '') => {
    try {
      ...
      const response = await axios.get(
        `http://54.236.98.193:8132/api/damage?page=${page}&limit=${limit}&search=${search}`
      );
  ...
  const handleDeleteDamageManage = async (id) => {
    try {
      await axios.delete(`http://54.236.98.193:8132/api/damage/${id}`);
  ...
  const confirmRefund = async () => {
    ...
    try {
      ...
      const response = await axios.post(`http://54.236.98.193:8132/api/damage/refund/${selectedDamage.paymentId}`);
  ...
  const handleGeneratePDF = async (damageId) => {
    try {
      const response = await axios.post('http://54.236.98.193:8132/api/damage/send-damage-report', { damageId }, { responseType: 'blob' });
  ...
  const handleViewDamage = async (damage) => {
    ...
    try {
      const response = await axios.get(`http://54.236.98.193:8132/api/damage/${damage._id}`);
      if (response.data.success) {
