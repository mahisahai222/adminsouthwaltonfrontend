...
const Feedback = () => {
  ...
  const fetchFeedbackData = async () => {
    try {
      const response = await axios.get('http://localhost:8132/api/feedback');
  ...
  const handleAddFeedback = async () => {
    ...
    try {
      await axios.post('http://localhost:8132/api/feedback/add', feedback);
      fetchFeedbackData();
      resetForm();
      setVisible(false);
  ...
  const handleDeleteFeedback = async (id) => {
    try {
      await axios.delete(`http://localhost:8132/api/feedback/${id}`);
