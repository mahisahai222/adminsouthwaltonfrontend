...
const OrderStatusTrack = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      const response = await axios.get('http://44.196.192.232:8132/api/OrderDetails/status/ACCEPT');
  ...
  const handleUpdateTrackStatus = async () => {
    try {
      await axios.put(`http://44.196.192.232:8132/api/OrderStatusTrack/${selectedOrder._id}`, { trackStatus: newTrackStatus });
  ...
  const handleDeleteOrder = async (orderId) => {
    try {
      await axios.delete(`http://44.196.192.232:8132/api/OrderDetails/${orderId}`);
