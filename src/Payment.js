...
const Payment = () => {
  ...
  const fetchPaymentData = async (page = 1, email = "") => {
    setLoading(true);
    try {
      const response = await axios.get("http://44.196.64.110:8132/api/pay/pays", {
  ...
  const handleDeletePayment = async (id) => {
    try {
      await axios.delete(`http://44.196.64.110:8132/api/pay/${id}`);
  ...
  const handleViewPayment = async (payment) => {
    ...
    try {
      const response = await axios.get(`http://44.196.64.110:8132/api/pay/payment/${payment._id}`);
      if (response) {
