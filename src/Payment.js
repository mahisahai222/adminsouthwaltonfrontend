...
const Payment = () => {
  ...
  const fetchPaymentData = async (page = 1, email = "") => {
    setLoading(true);
    try {
      const response = await axios.get("http://54.236.98.193:8132/api/pay/pays", {
  ...
  const handleDeletePayment = async (id) => {
    try {
      await axios.delete(`http://54.236.98.193:8132/api/pay/${id}`);
  ...
  const handleViewPayment = async (payment) => {
    ...
    try {
      const response = await axios.get(`http://54.236.98.193:8132/api/pay/payment/${payment._id}`);
      if (response) {
