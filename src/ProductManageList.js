...
const ProductManageList = () => {
  ...
  const fetchProductManageData = async () => {
    try {
      const response = await axios.get('http://44.196.192.232:8132/api/ProductManageData')
  ...
  const handleAddProductManage = async () => {
    ...
    try {
      const response = await axios.post(
        'http://44.196.192.232:8132/api/ProductManageData/pname',
        formData,
        {
          headers: {
  ...
  const handleDeleteProductManage = async (id) => {
    try {
      await axios.delete(`http://44.196.192.232:8132/api/ProductManageData/${id}`)
  ...
  const handleUpdateProductManage = async () => {
    ...
    try {
      const response = await axios.put(
        `http://44.196.192.232:8132/api/ProductManageData/${currentProductId}`,
        formData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
                ...
                {productManageData.map((product) => (
                  <CTableRow key={product._id}>
                    ...
                    <CTableDataCell>
                      {product.image && (
                        <img
                          src={`http://44.196.192.232:8132/uploads/${product.image}`}
