...
const Season = () => {
  ...
  const fetchSeasons = async () => {
    try {
      const response = await axios.get('http://44.196.64.110:8132/api/seasons');
  ...
  const handleSaveSeason = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(
          `http://44.196.64.110:8132/api/seasons/${seasonId}/${editEntryId}`,
          seasonEntry
        );
      } else {
        await axios.post(`http://44.196.64.110:8132/api/seasons/${seasonId}/add-entry`, seasonEntry);
      }
      fetchSeasons();  
  ...
  const handleDeleteSeason = async (seasonType, entryId) => {
    try {
      ...
      await axios.delete(`http://44.196.64.110:8132/api/seasons/${seasonId}/${seasonType}/${entryId}`);
      fetchSeasons();
