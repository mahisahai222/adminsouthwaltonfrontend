...
const StaticContentManageList = () => {
  ...
  const handleAddStaticContent = async () => {
    try {
      const response = await axios.post('http://54.236.98.193:8132/api/StaticContentManageData/StaticContent', { policy, termsandconditions });
  ...
  const handleUpdateStaticContent = async () => {
    try {
      ...
      const response = await axios.put(`http://54.236.98.193:8132/api/StaticContentManageData/${id}`, { policy, termsandconditions });
