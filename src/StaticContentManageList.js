...
const StaticContentManageList = () => {
  ...
  const handleAddStaticContent = async () => {
    try {
      const response = await axios.post('http://localhost:8132/api/StaticContentManageData/StaticContent', { policy, termsandconditions });
  ...
  const handleUpdateStaticContent = async () => {
    try {
      ...
      const response = await axios.put(`http://localhost:8132/api/StaticContentManageData/${id}`, { policy, termsandconditions });
