...
const StaticContentManageList = () => {
  ...
  const handleAddStaticContent = async () => {
    try {
      const response = await axios.post('http://3.223.253.106:8132/api/StaticContentManageData/StaticContent', { policy, termsandconditions });
  ...
  const handleUpdateStaticContent = async () => {
    try {
      ...
      const response = await axios.put(`http://3.223.253.106:8132/api/StaticContentManageData/${id}`, { policy, termsandconditions });
