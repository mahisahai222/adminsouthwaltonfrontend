...
const StaticContentManageList = () => {
  ...
  const handleAddStaticContent = async () => {
    try {
      const response = await axios.post('http://18.209.197.35:8132/api/StaticContentManageData/StaticContent', { policy, termsandconditions });
  ...
  const handleUpdateStaticContent = async () => {
    try {
      ...
      const response = await axios.put(`http://18.209.197.35:8132/api/StaticContentManageData/${id}`, { policy, termsandconditions });
