...
const TaskManageList = () => {
  ...
  const fetchTaskData = async () => {
    try {
      const response = await axios.get('http://3.111.163.2:8132/api/task');
  ...
  const handleAddTask = async () => {
    try {
      const response = await axios.post(
        'http://3.111.163.2:8132/api/task/add',
        {
          driverName,
          customerName,
          pickupAddress,
  ...
  const handleUpdateTask = async () => {
    try {
      const response = await axios.put(
        `http://3.111.163.2:8132/api/task/${currentTaskId}`,
        {
          driverName,
          customerName,
          pickupAddress,
  ...
  const handleDeleteTask = async (id) => {
    try {
      await axios.delete(`http://3.111.163.2:8132/api/task/${id}`);
