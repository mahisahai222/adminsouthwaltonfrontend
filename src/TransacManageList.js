...
const TransacManageList = () => {
  ...
  useEffect(() => {
    const fetchTransacManageData = async () => {
      try {
        const response = await axios.get('http://3.111.163.2:8132/api/TransacManageData/');
  ...
  const handleAddTransacManage = async () => {
    try {
      const response = await axios.post('http://3.111.163.2:8132/api/TransacManageData/password', formData);
  ...
  const handleDeleteTransacManage = async (id) => {
    try {
      await axios.delete(`http://3.111.163.2:8132/api/TransacManageData/${id}`);
