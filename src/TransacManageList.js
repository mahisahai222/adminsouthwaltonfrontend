...
const TransacManageList = () => {
  ...
  useEffect(() => {
    const fetchTransacManageData = async () => {
      try {
        const response = await axios.get('http://44.196.64.110:8132/api/TransacManageData/');
  ...
  const handleAddTransacManage = async () => {
    try {
      const response = await axios.post('http://44.196.64.110:8132/api/TransacManageData/password', formData);
  ...
  const handleDeleteTransacManage = async (id) => {
    try {
      await axios.delete(`http://44.196.64.110:8132/api/TransacManageData/${id}`);
