...
const TransacManageList = () => {
  ...
  useEffect(() => {
    const fetchTransacManageData = async () => {
      try {
        const response = await axios.get('http://18.209.197.35:8132/api/TransacManageData/');
  ...
  const handleAddTransacManage = async () => {
    try {
      const response = await axios.post('http://18.209.197.35:8132/api/TransacManageData/password', formData);
  ...
  const handleDeleteTransacManage = async (id) => {
    try {
      await axios.delete(`http://18.209.197.35:8132/api/TransacManageData/${id}`);
