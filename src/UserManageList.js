...
const UserManageList = () => {
  ...
  const fetchUserManageData = async (page = 1, query = '') => {
    setLoading(true);
    try {
      const response = await axios.get('http://54.236.98.193:8132/api/user/', {
        params: {
          page,
  ...
  const handleAddUserManage = async () => {
    ...
    try {
      const response = await axios.post('http://54.236.98.193:8132/api/user/register', formData, {
        headers: {
  ...
  const handleUpdateUserManage = async () => {
    ...
    try {
      const response = await axios.put(`http://54.236.98.193:8132/api/user/${editUserId}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleDeleteUserManage = async (id) => {
    try {
      await axios.delete(`http://54.236.98.193:8132/api/user/${id}`);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    try {
      const response = await axios.post('http://54.236.98.193:8132/api/user/status', data);
