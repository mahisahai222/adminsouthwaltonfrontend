...
const UserManageList = () => {
  ...
  const fetchUserManageData = async () => {
    try {
      const response = await axios.get('http://localhost:8132/api/user/');
  ...
  const handleAddUserManage = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8132/api/user/register', formData, {
        headers: {
  ...
  const handleUpdateUserManage = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:8132/api/user/${editUserId}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleDeleteUserManage = async (id) => {
    try {
      await axios.delete(`http://localhost:8132/api/user/${id}`);
...
const handleToggleStatus = async (id, currentStatus) => {
  ...
  try {
    const response = await axios.post('http://localhost:8132/api/user/status', data);
