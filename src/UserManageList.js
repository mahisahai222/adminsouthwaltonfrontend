...
const UserManageList = () => {
  ...
  const handleAddUserManage = async () => {
    ...
    try {
      const response = await axios.post('http://44.196.64.110:8132/api/user/register', formData, {
        headers: {
  ...
  const handleUpdateUserManage = async () => {
    ...
    try {
      const response = await axios.put(`http://44.196.64.110:8132/api/user/${editUserId}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleDeleteUserManage = async (id) => {
    try {
      await axios.delete(`http://44.196.64.110:8132/api/user/${id}`);
