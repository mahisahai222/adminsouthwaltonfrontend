...
const VehicleManageList = () => {
  ...
  const fetchVehicleData = async () => {
    try {
      const response = await axios.get(`http://3.223.253.106:8132/api/vehicle`, {
  ...
  const handleAddVehicle = async () => {
    ...
    try {
      const response = await axios.post('http://3.223.253.106:8132/api/vehicle/add', formData, {
        headers: {
  ...
  const handleUpdateVehicle = async () => {
    ...
    try {
      const response = await axios.put(`http://3.223.253.106:8132/api/vehicle/${currentVehicleId}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleDeleteVehicle = async (id) => {
    try {
      await axios.delete(`http://3.223.253.106:8132/api/vehicle/${id}`);
