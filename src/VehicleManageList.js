...
const VehicleManageList = () => {
  ...
  const fetchVehicleData = async () => {
    try {
      const response = await axios.get(`http://54.236.98.193:8132/api/vehicle`, {
  ...
  const handleAddVehicle = async () => {
    ...
    try {
      const response = await axios.post('http://54.236.98.193:8132/api/vehicle/add', formData, {
        headers: {
  ...
  const handleUpdateVehicle = async () => {
    ...
    try {
      const response = await axios.put(`http://54.236.98.193:8132/api/vehicle/${currentVehicleId}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleDeleteVehicle = async (id) => {
    try {
      await axios.delete(`http://54.236.98.193:8132/api/vehicle/${id}`);
