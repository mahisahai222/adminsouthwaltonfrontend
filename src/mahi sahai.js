...
const Season = () => {
  ...
  const handleSaveSeason = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`http://44.196.64.110:8132/api/seasons/${editEntryId}`, seasonEntry);
      } else {
        await axios.post('http://44.196.64.110:8132/api/seasons/add', seasonEntry);
      }
      fetchSeasons();
  ...
  const handleDeleteSeason = async (id) => {
    try {
      await axios.delete(`http://44.196.64.110:8132/api/seasons/${id}`);
      fetchSeasons();
    } catch (error) {
