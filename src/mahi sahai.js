...
const Season = () => {
  ...
  const handleSaveSeason = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`http://54.236.98.193:8132/api/seasons/${editEntryId}`, seasonEntry);
      } else {
        await axios.post('http://54.236.98.193:8132/api/seasons/add', seasonEntry);
      }
      fetchSeasons();
  ...
  const handleDeleteSeason = async (id) => {
    try {
      await axios.delete(`http://54.236.98.193:8132/api/seasons/${id}`);
      fetchSeasons();
    } catch (error) {
