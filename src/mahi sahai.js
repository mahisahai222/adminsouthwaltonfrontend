...
const Season = () => {
  ...
  const handleSaveSeason = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`http://18.209.197.35:8132/api/seasons/${editEntryId}`, seasonEntry);
      } else {
        await axios.post('http://18.209.197.35:8132/api/seasons/add', seasonEntry);
      }
      fetchSeasons();
  ...
  const handleDeleteSeason = async (id) => {
    try {
      await axios.delete(`http://18.209.197.35:8132/api/seasons/${id}`);
      fetchSeasons();
    } catch (error) {
