...
const VerifyOTP = () => {
  ...
  const handleVerifyOTP = async () => {
    try {
      const response = await axios.post('http://44.196.192.232:8132/api/auth/verify-otp', { email, otp });
      setMessage(response.data.message);
      if (response.status === 200) {
        window.location.href = 'http://44.196.192.232:3000/reset-password'; // Redirect to reset password page
      }
    } catch (error) {
